...
export default function DeviceStatusPage({ onNavigate }: DeviceStatusPageProps) {
  ...
  // Filter devices based on status, search query, and building
  const filteredDevices = allDevices.filter(device => {
    // Status filter
    const matchesStatus = statusFilter === 'All' || 
                         device.status === statusFilter.toLowerCase() || 
                         device.operate === statusFilter;
    
    // Search query filter (search in device name)
    ...
    const matchesBuilding = buildingFilter === '' || 
                           device.buildingName.toLowerCase().includes(buildingFilter.toLowerCase());
    
    return matchesStatus && matchesSearch && matchesBuilding;
  });

  const handleSearch = () => {
    // The filtering happens automatically through the filteredDevices calculation
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="bg-gray-800 text-white text-sm rounded-lg px-3 py-2 border border-gray-700 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                >
              ...
              <div className="flex-1 min-w-[200px]">
                <input
                  type="text"
                  placeholder="Search device or building..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  onKeyPress={(e) => e.key === 'Enter' && handleSearch()}
                  className="w-full bg-gray-800 text-white text-sm rounded-lg px-4 py-2 border border-gray-700 focus:ring-2 focus:ring-blue-500 focus:outline-none placeholder-gray-500"
                />
              </div>
                ...
                <tbody>
                  {filteredDevices.map((device, index) => (
                    <tr 
                      ...
                      onClick={() => setBuildingFilter(device.buildingName)}
                    >
                      ...
                      <td className="py-4 px-4">
                        <span className={`inline-flex items-center gap-1 text-xs font-medium ${
                          device.operate === 'Using' ? 'text-red-400' : 'text-gray-400'
                        }`}>
                      ...
                      <td className="py-4 px-4">
                        <button 
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            console.log('View device:', device.deviceName);
                          }}
                        >
                          <BarChart3 size={20} />
                        </button>
